    ...
    <div className="max-w-7xl mx-auto px-8 py-12">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-64 flex-shrink-0 space-y-8">
          <div>
            <h3 className="font-manrope text-headline-md text-[#004c4c] mb-4">Kategorien</h3>
            <ul className="space-y-2">
              {categories.map((cat) => (
                <li key={cat}>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" className="w-5 h-5 rounded border-[#6f7979] text-[#004c4c] focus:ring-[#006666]" />
                    <span className="text-body-md group-hover:text-[#004c4c] transition-colors">{cat}</span>
                  </label>
                </li>
              ))}
            </ul>
          </div>
          <div className="pt-6 border-t border-[#bec9c8]">
            <h3 className="font-manrope text-headline-md text-[#004c4c] mb-4">Preisbereich</h3>
            <input type="range" min="0" max="1000" className="w-full h-2 bg-[#dee8ff] rounded-lg appearance-none cursor-pointer accent-[#004c4c]" />
            <div className="flex justify-between text-body-sm text-[#515f78] mt-2">
              <span>0 €</span>
              <span>1.000 €</span>
            </div>
          </div>
          <button className="w-full py-3 bg-[#004c4c] text-white font-bold rounded-lg hover:brightness-110 transition-all">
            Filter anwenden
          </button>
        </aside>
        ...
        <section className="flex-grow">
          <header className="flex justify-between items-end mb-8">
            <div>
              <h1 className="font-manrope text-headline-xl text-[#004c4c] tracking-tight">Unsere Produkte</h1>
              <p className="text-[#515f78] text-body-lg mt-2">Hochwertige medizinische Hilfsmittel für Ihren Alltag.</p>
            </div>
            <div className="flex items-center gap-2 text-body-sm text-[#515f78]">
              <span>Sortieren nach:</span>
              <select className="bg-transparent border-none font-bold text-[#004c4c] focus:ring-0 cursor-pointer">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {products.map((p) => (
              <article key={p.id} className="bg-white border border-[#bec9c8] rounded-xl overflow-hidden group hover:shadow-lg transition-all duration-300">
                <div className="aspect-square bg-white relative overflow-hidden">
                  <img src={p.image} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  {p.badge && (
                    <span className="absolute top-4 right-4 bg-[#006666] text-white px-3 py-1 rounded-full text-xs font-bold uppercase">{p.badge}</span>
                  )}
                </div>
                <div className="p-6">
                  <span className="text-xs font-bold text-[#004c4c] mb-1 block uppercase">{p.category}</span>
                  <h2 className="font-manrope text-headline-md text-[#004c4c] mb-2">{p.name}</h2>
                  <p className="text-[#515f78] text-body-sm mb-4 line-clamp-2">{p.description}</p>
                  <div className="flex justify-between items-center mt-4">
                    <span className="font-manrope text-headline-md text-[#111c2c]">{p.price}</span>
                    <button className="p-3 bg-[#004c4c] text-white rounded-lg flex items-center gap-2 hover:bg-[#006666] transition-colors active:scale-95">
          ...
          <div className="mt-12 flex justify-center gap-2">
            {[1, 2, 3].map((n) => (
              <button key={n} className={`w-10 h-10 flex items-center justify-center rounded-lg border border-[#bec9c8] hover:bg-[#e7eeff] transition-colors ${n === 1 ? 'text-[#004c4c] font-bold' : ''}`}>{n}</button>
            ))}
            <span className="w-10 h-10 flex items-center justify-center">...</span>
            <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-[#bec9c8] hover:bg-[#e7eeff] transition-colors">
